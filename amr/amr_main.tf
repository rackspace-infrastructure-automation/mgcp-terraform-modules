...
module "gce_disk_usage" {
  ...
  runbook_content        = <<EOT
                           ...
                           EOT
  group_by_fields        = ["resource.label.project_id", "resource.label.zone", "resource.label.instance_id", "metric.label.device"]
  notification_channels  = [module.rackspace_urgent.notification_id]
  threshold              = 90
}
...
module "csql_disk_util" {
  source                 = "./modules/base-policy"
  combiner               = "OR"
  policy_display_name    = "rax-amr-monitoring-csql_disk_util"
  condition_display_name = "DISK UTILIZATION for CSQL INSTANCES"
  condition_filter       = <<EOT
...
module "csql_replica_lag" {
  ...
  runbook_content        = <<EOT
                           ...
                           2. Consider upgrading the replica node type, troubleshooting, or enabling Parralell replication MySQL 5.7 + 8.0 examples:
